...
import pymysql
con=pymysql.Connect(host='localhost',port=3306,user='root',passwd='',db='sams')
cmd=con.cursor()
...
def Recive():
    ...
    if connected:
        ...
        print(serialport.inWaiting(),"abcdefg")
...
def stud():
    ...
    sid= combo4.get()
    #print("sid=====",sid)
    cmd.execute("select `Name` from `myapp_staff`")
...
def insert():
    #print(combo4)
    sid=combo4.get()
    a=str(sid).replace('{','').replace('}','')
    # #print(a)
    #print(type(sid))
    # std =
    cmd.execute("select id from `myapp_staff` where `Name`='"+a+"'")
    ...
    fid=e1.get()
    #print(fid)
    cmd.execute("INSERT INTO `myapp_finger_#print` VALUES(NULL,'"+str(stid)+"','"+fid+"')")
...
comboExample = Combobox(root,values=["COM4", "COM5","COM6","COM7","COM10"] )
